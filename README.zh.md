...
插件会在 Web 界面中以居中、等比例方式显示配置的产品图标，并单独把 favicon 和 PWA 图标包装为透明的方形 SVG 画布；非方形图标会留白并保持原始比例，不会拉伸。HTTPS 图片会由插件缓存并通过固定的同源路径提供给浏览器元数据。DSH 运行期显示会话标题时，标题后缀仍会保持为配置的产品名称。
